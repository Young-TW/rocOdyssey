...
__device__ double task1fun_GetZ(double* Variables, double* VariablesIn,
                                double* y) {
    double r1 = y[0];
    double E_local = -(r1 * r1 + A * sqrt(r1)) /
                         (r1 * sqrt(r1 * r1 - 3. * r1 + 2. * A * sqrt(r1))) +
                     L / sqrt(r1) / sqrt(r1 * r1 - 3. * r1 + 2. * A * sqrt(r1));
    double E_inf = -1.0;
    return E_local / E_inf;
}

__global__ void GPU_task1work(double* ResultsPixel, double* VariablesIn,
...
//
// =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
__device__ double task2fun_GetZ(double* Variables, double* VariablesIn,
                                double* y) {
    ...
    double ut_k =
        (r * r + A * sqrt(r)) / (r * sqrt(r * r - 3. * r + 2. * A * sqrt(r)));
    double ur_k = 0.;
    ...
    if (r < Rmstable) {
        ...
        double lambda =
            (Rmstable * Rmstable - 2. * A * sqrt(Rmstable) + a2) /
            (sqrt(Rmstable * Rmstable * Rmstable) - 2. * sqrt(Rmstable) + A);
...
__global__ void GPU_task2work(double* ResultsPixel, double* VariablesIn,
                              int GridIdxX, int GridIdxY) {
    if (X1 >= SIZE || Y1 >= SIZE) return;
    ...
    while (1) {
        ...
        if (y[0] < Rdisk) {
            double zzz =
                task2fun_GetZ(Variables, VariablesIn, y);  // zzz=E_em/E_obs
            ...
            double comp1 =
                4. * PI * nth * C_e * C_e * v / sqrt(3.) / K_value / C_c;
            double comp2 = 4.0505 / pow(x, (1. / 6.)) *
                           (1. + 0.4 / pow(x, 0.25) + 0.5316 / sqrt(x)) *
                           exp(-1.8899 * pow(x, 1. / 3.));
            double j_nu = comp1 * comp2;
            double B_nu = 2.0 * v * v * v * C_h / C_c / C_c /
                          (exp(C_h * v / C_kB / Te) - 1.0);
            ...
            dI = dI + ds * C_sgrA_mbh * C_rgeo * j_nu / freq_local /
                          freq_local / freq_local * exp(-dtau) * zzz;
        }
