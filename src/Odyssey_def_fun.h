/***********************************************************************************
    ...
    This file is part of Odyssey source code. Odyssey is a public, GPU-based
code for General Relativistic Radiative Transfer (GRRT), following the
    ...
    We ask that users of Odyssey cite the following paper in their subsequent
scientific literature and publications which result from the use of any part of
Odyssey: "Odyssey: A PUBLIC GPU-BASED CODE FOR GENERAL-RELATIVISTIC RADIATIVE
TRANSFER IN KERR SPACE-TIME" by Hung-Yi Pu, Kiyun Yun, Ziri Younsi, and Suk-Jin
...
__device__ static void geodesic(double *Variables, double *VariablesIn,
    ...
    dydx[4] = -(((r - 1.0) * (-kappa) + twor * (r2 + a2) - 2.0 * A * L) * bot -
...
__device__ static void rkstep(double *Variables, double *VariablesIn, double *y,
                              double *dydx, double h, double *yout,
                              double *yerr) {
    int i;
    double ak[N];
...
__device__ static double rk5(double *Variables, double *VariablesIn, double *y,
                             double *dydx, double htry, double escal,
...
__device__ static void initial(double *Variables, double *VariablesIn,
    ...
    double rdot0 =
        zdot * (-u * u * cos(theta0) * cos(theta) + r * u * v * sin(theta)) /
        sigma;
    double thetadot0 =
        zdot * (cos(theta0) * r * sin(theta) + u * v * cos(theta)) / sigma;
...
__device__ static float ISCO(double *VariablesIn) {
    double z1 = 1 + pow(1 - A * A, 1 / 3.0) *
                        (pow(1 + A, 1 / 3.0) + pow(1 - A, 1 / 3.0));
...
static __device__ __constant__ double K2_tab[] = {
    -10.747001,  // Te=0.1
    -9.362569,  -8.141373, -7.061568, -6.104060, -5.252153, -4.491244,
    -3.808555,  -3.192909, -2.634534, -2.124893, -1.656543, -1.223007,
    -0.818668,  -0.438676, -0.078863, +0.264332, +0.593930, +0.912476,
    +1.222098,  +1.524560, +1.821311, +2.113537, +2.402193, +2.688050,
    +2.971721,  +3.253692, +3.534347, +3.813984, +4.092839, +4.371092,
    +4.648884,  +4.926323, +5.203493, +5.480457, +5.757264, +6.033952,
    +6.310550,  +6.587078, +6.863554, +7.139990, +7.416395, +7.692778,
    +7.969143,  +8.245495, +8.521837, +8.798171, +9.074500, +9.350824,
    +9.627144  // Te=87.09
};
...
__device__ static double Jansky_Correction(double *VariablesIn,
                                           double ima_width) {
    double distance = C_sgrA_d * C_pc;
    double theta = atan(ima_width * C_sgrA_mbh * C_rgeo / distance);
    double pix_str = theta / (SIZE / 2.) * theta /
                     (SIZE / 2.);  // radian^2 (steradians) per pixel
    return pix_str / C_Jansky;
}

__device__ static double Luminosity_Correction(double *VariablesIn,
                                               double ima_width) {
    double distance = C_sgrA_d * C_pc;
    double theta = atan(ima_width * C_sgrA_mbh * C_rgeo / distance);
    double pix_str = theta / (SIZE / 2.) * theta /
